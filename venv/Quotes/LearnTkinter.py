...
def overlap_process(event):
    ...
    if overlap == '布林线':
        ...
        real = ta.HT_TRENDLINE(close, timeperiod=30)
        axes[1].plot(real, 'r-')
        ...
        mama, fama = ta.MAMA(close, timeperiod=30, slowlimit=0)
        axes[1].plot(mama, 'r-')
        axes[1].plot(fama, 'g-')
...
def momentum_process(event):
    ...
    if momentum == '绝对价格振荡器':
        ...
        fastk, fastd = ta.STROCHRSI(close, timeperiod=14, fastk_period=5, fasted_period=3, fasted_matype=0)
        axes[1].plot(fastk, 'r-')
        axes[1].plot(fastd, 'r-')
...
def math_operator_process(event):
    ...
    if math_operator == '指定的区间最大值':
        real = ta.MAX(close, timeperiod=30)
        axes[1].plot(real, 'r-')
    elif math_operator == '指定的区间最大值的索引':
        integer = ta.MAXINDEX(close, timeperiod=30)
        axes[1].plot(integer, 'r-')
    elif math_operator == '指定的区间最小值':
        real = ta.MIN(close, timeperiod=30)
        axes[1].plot(real, 'r-')
    elif math_operator == '指定的区间最小值的索引':
        integer = ta.MININDEX(close, timeperiod=30)
        axes[1].plot(integer, 'r-')
    elif math_operator == '指定的区间最小值和最大值':
        real = ta.MINMAX(close, timeperiod=30)
        axes[1].plot(real, 'r-')
    elif math_operator == '指定的区间最小值和最大值的索引':
